...
function formatAction(action: string) {
  const labels: Record<string, string> = {
    ...
    provider_channel_handle_updated: "Provider account mapping updated",
...
function getChannelReadinessItems(channel: Channel) {
  return [
    ["Provider ID", Boolean(channel.readiness?.providerIdSaved)],
    ["Webhook", Boolean(channel.readiness?.webhookTested)],
    ["Recipient", Boolean(channel.readiness?.recipientMapped)],
...
export function ClientBusinessDetail({ businessId }: { businessId: string }) {
  ...
  const [channelHandleEdits, setChannelHandleEdits] = useState<Record<string, string>>({});
  ...
  const loadProviderReadiness = useCallback(async () => {
    ...
    const response = await fetch("/api/admin/provider-readiness", {
      headers: { Authorization: `Bearer ${session.access_token}` },
    });
    const result = (await response.json()) as ProviderReadiness & { error?: string };
  ...
  useEffect(() => {
    ...
    void Promise.resolve().then(() => loadProviderReadiness());

    return () => {
      mounted = false;
    };
  }, [loadApiSetups, loadDetail, loadNotes, loadProviderReadiness]);
  ...
  const updateChannelHandle = async (channelId: string, handle: string) => {
    ...
    try {
      ...
      const response = await fetch(`/api/admin/business/${businessId}/channels/${channelId}`, {
        method: "PATCH",
        headers: {
          Authorization: `Bearer ${session.access_token}`,
          "Content-Type": "application/json",
      ...
      if (!response.ok || !result.ok) {
        throw new Error(result.error ?? "Could not update provider account id.");
      }

      const nextData = await loadDetail();
      if (nextData.data) setData(nextData.data);
      setActionMessage("Provider account id saved. Webhook mapping is ready for this channel.");
    } catch (handleError) {
      setActionMessage(handleError instanceof Error ? handleError.message : "Could not update provider account id.");
    } finally {
      setBusyChannelId("");
    }
  };
  ...
  const metaReadiness = providerReadiness.meta;
  const configuredMetaItems = [
    metaReadiness?.appIdConfigured,
    metaReadiness?.appSecretConfigured,
    metaReadiness?.verifyTokenConfigured,
    metaReadiness?.tokenEncryptionConfigured,
    ...
    <main className="min-h-screen bg-[#030712] px-4 py-6 text-white">
      <div className="mx-auto w-full max-w-7xl">
          ...
          <div className="rounded-lg border border-white/10 bg-[#07101d]/90 p-5">
            ...
            <div className="mt-5 grid gap-3">
              {channels.map((channel) => {
                ...
                const liveReady = Boolean(channel.readiness?.liveReady);
                return (
                  <div key={channel.id} className="grid gap-3 rounded-lg bg-white/[0.035] p-4">
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                      ...
                      <span className={`w-fit rounded-full px-3 py-1 text-xs font-bold ${liveReady ? "bg-blue-400/10 text-blue-200" : "bg-amber-300/10 text-amber-100"}`}>
                        {liveReady ? "live ready" : "setup pending"}
                      </span>
                    </div>
                    <div className="grid gap-2 sm:grid-cols-[1fr_auto]">
                      <input
                        value={channelHandleEdits[channel.id] ?? channel.handle ?? ""}
                        onChange={(event) =>
                          setChannelHandleEdits((current) => ({
                            ...current,
                            [channel.id]: event.target.value,
                          }))
                        }
                        placeholder="Provider account ID / webhook entry.id"
                        className="h-10 min-w-0 rounded-lg border border-white/10 bg-[#030712] px-3 text-xs text-slate-100 outline-none transition placeholder:text-slate-600 hover:border-blue-300/40 focus:border-blue-300/60"
                      />
                      <button
                        onClick={() => void updateChannelHandle(channel.id, channelHandleEdits[channel.id] ?? channel.handle ?? "")}
                        disabled={busyChannelId === channel.id}
                        className="h-10 rounded-lg border border-blue-300/20 bg-blue-400/10 px-3 text-xs font-bold text-blue-100 transition hover:bg-blue-400/15 disabled:cursor-not-allowed disabled:opacity-50"
                      >
                        {busyChannelId === channel.id ? "Saving..." : "Save ID"}
                      </button>
                    </div>
                    <p className="text-[11px] leading-5 text-slate-500">
                      Match this with Meta webhook entry.id so real events can route into the client inbox.
                    </p>
                    <div className="grid grid-cols-2 gap-2">
                      {getChannelReadinessItems(channel).map(([label, complete]) => (
                        <div key={label} className="rounded-md border border-white/10 bg-[#030712]/60 px-2 py-2 text-[10px]">
                    ...
                    <Link
                      ...
                      className="rounded-lg border border-blue-300/20 bg-blue-400/10 px-3 py-2 text-center text-xs font-bold text-blue-100 transition hover:bg-blue-400/15"
                    >
                      Open Provider Events
                    </Link>
                  </div>
                );
              })}
        ...
        <section className="mt-6 rounded-lg border border-white/10 bg-[#07101d]/90 p-5">
          ...
          <div className="mt-5 grid gap-4 rounded-lg border border-blue-300/15 bg-blue-400/10 p-4 lg:grid-cols-[1fr_auto] lg:items-center">
            <div className="min-w-0">
              <p className="text-sm font-black uppercase tracking-[0.22em] text-blue-200">Meta webhook readiness</p>
              <p className="mt-2 break-all rounded-lg border border-white/10 bg-[#030712]/70 p-3 text-xs font-semibold text-slate-200">
                {metaReadiness?.callbackUrl ?? "https://pasnex.com/api/provider/meta/webhook"}
              </p>
              <p className="mt-2 text-xs leading-5 text-slate-400">
                Use this HTTPS callback URL in Meta App Dashboard. Verification works only after production env has a matching verify token.
              </p>
            </div>
            <div className="grid gap-2 text-xs font-bold sm:grid-cols-2 lg:w-[430px]">
              {[
                ["App ID", metaReadiness?.appIdConfigured],
                ["App Secret", metaReadiness?.appSecretConfigured],
                ["Verify Token", metaReadiness?.verifyTokenConfigured],
                ["Token Encryption", metaReadiness?.tokenEncryptionConfigured],
              ].map(([label, ready]) => (
                <div key={String(label)} className={`rounded-lg border p-3 text-center ${ready ? "border-blue-300/20 bg-blue-400/10 text-blue-100" : "border-amber-300/20 bg-amber-300/10 text-amber-50"}`}>
                  <p>{label}</p>
                  <p className="mt-1 text-[10px] uppercase tracking-wider">{ready ? "configured" : "missing"}</p>
                </div>
              ))}
              <Link href="/admin/launch-checklist" className="rounded-lg border border-white/10 bg-white/[0.04] p-3 text-center text-blue-200 transition hover:border-blue-300/40 hover:bg-white/[0.08] sm:col-span-2">
                Review launch checklist - {configuredMetaItems}/4 ready
              </Link>
            </div>
          </div>
          {apiSetupMessage && (
