...
const faqs = [
  ...
  {
    question: "Can any type of business use Pasnex.ai?",
    answer: "Yes. Pasnex.ai is built for any business that receives customer enquiries, support questions, booking requests, or sales conversations through social and messaging channels.",
  },
  {
  ...
  {
    question: "Are Instagram and WhatsApp messages live immediately?",
    answer: "The workspace can be prepared immediately. Real live messaging starts after official provider approval, API setup, webhooks, and token configuration are completed.",
  },
  {
    question: "What happens during onboarding?",
    answer: "You choose your primary channel, prepare the workspace, create your first automation, and enter the dashboard. Real provider connection is completed after approval and setup.",
  },
  {
    question: "How does Pasnex.ai find real customers?",
    answer: "Pasnex.ai captures enquiries, understands customer intent, scores lead quality, and helps your team focus on serious prospects instead of casual messages.",
  },
  {
    question: "Is customer data safe?",
    answer: "Pasnex.ai is designed with secure workspaces, role-based access, internal admin tracking, and careful data handling. Production integrations will follow provider and privacy requirements.",
  },
  {
    question: "Can my team review AI replies before sending?",
    answer: "Yes. AI can prepare suggested replies, but your team can review, edit, approve, and handle important conversations before moving to full automation.",
  },
  {
  ...
  },
  {
    ...
    answer: "Register, verify your email, complete onboarding, and run a trial automation. After that, the Pasnex.ai team can help prepare real channel API approvals and production setup.",
  },
];

const aiDemoMessages = [
  { role: "ai", text: "Welcome to Pasnex AI. How can I automate your business today?" },
  { role: "user", text: "A customer commented: price details please." },
  { role: "ai", text: "I detected pricing intent and prepared a friendly DM reply." },
  { role: "ai", text: "Reply: Thanks for your interest. Please share your phone number and requirement so our team can help you." },
  { role: "user", text: "The customer shared their phone number." },
  { role: "ai", text: "Lead captured. Score updated to 86/100 because the customer asked about price and shared contact details." },
  { role: "ai", text: "Next action created: send demo link and pricing options. Your team can review it in Inbox and Contacts." },
];
...
export default function Home() {
  ...
  const [aiDemoStarted, setAiDemoStarted] = useState(false);
  const [visibleAiMessages, setVisibleAiMessages] = useState(aiDemoMessages.length);
  const [activeSection, setActiveSection] = useState("home");
  const aiChatRef = useRef<HTMLDivElement>(null);
  const aiScrollRef = useRef<HTMLDivElement>(null);
  const formRedirectRef = useRef<HTMLInputElement>(null);
  const formEmailRef = useRef<HTMLInputElement>(null);
  const formCountryCodeRef = useRef<HTMLSelectElement>(null);
  const formPhoneRef = useRef<HTMLInputElement>(null);
  const formReplyToRef = useRef<HTMLInputElement>(null);
  const formContactSummaryRef = useRef<HTMLInputElement>(null);
  ...
  useEffect(() => {
    if (!aiDemoStarted || visibleAiMessages >= aiDemoMessages.length) return;

    const timer = window.setTimeout(() => {
      setVisibleAiMessages((count) => Math.min(count + 1, aiDemoMessages.length));
    }, 950);

    return () => window.clearTimeout(timer);
  }, [aiDemoStarted, visibleAiMessages]);
  ...
  useEffect(() => {
    const sectionIds = ["home", "features", "solutions", "pricing", "about", "contact", "blog"];
    ...
    const updateActiveSection = () => {
      const currentSection =
        sectionIds
          ...
          .sort((a, b) => b.top - a.top)[0]?.id ?? "home";
  ...
  const navLinkClass = (sectionId: string) =>
    `rounded-full px-3 py-2 text-[15px] font-medium transition ${
      activeSection === sectionId ? "bg-blue-400/10 text-white" : "text-slate-300 hover:bg-white/[0.05] hover:text-white"
    }`;
  ...
  return (
    ...
    >
      <header className="fixed inset-x-0 top-3 z-50 px-3 sm:top-5 sm:px-4">
        <div className="mx-auto flex h-[68px] w-full max-w-7xl items-center justify-between rounded-2xl border border-white/10 bg-[#07101d]/90 px-4 shadow-2xl backdrop-blur-2xl sm:h-[76px] md:px-8">
          <button type="button" onClick={() => scrollToPageSection("home")} className="flex items-center gap-3 text-left">
          ...
          <nav className="hidden items-center gap-5 lg:flex">
            <button type="button" onClick={() => scrollToPageSection("home")} className={navLinkClass("home")}>Home</button>
            <button type="button" onClick={() => scrollToPageSection("features")} className={navLinkClass("features")}>Features</button>
            <button type="button" onClick={() => scrollToPageSection("solutions")} className={navLinkClass("solutions")}>Solutions</button>
            <button type="button" onClick={() => { trackCta("pricing", "desktop_nav", "pricing_click"); scrollToPageSection("pricing"); }} className={navLinkClass("pricing")}>Pricing</button>
            <button type="button" onClick={() => scrollToPageSection("about")} className={navLinkClass("about")}>About</button>
            <button type="button" onClick={() => scrollToPageSection("contact")} className={navLinkClass("contact")}>Contact</button>
            <button type="button" onClick={() => scrollToPageSection("blog")} className={navLinkClass("blog")}>Blog</button>
            <a href="/register" onClick={() => trackCta("register", "desktop_nav", "register_click")} className="text-[15px] font-medium text-slate-300 transition hover:text-white">Register</a>
            <a href="/login" onClick={() => trackCta("login", "desktop_nav")} className="text-[15px] font-medium text-slate-300 transition hover:text-white">Login</a>
          </nav>

          <div className="hidden items-center gap-4 md:flex">
            <a href="/register" onClick={() => trackCta("get_started", "header_nav", "register_click")} className="rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 px-6 py-3 text-[15px] font-bold shadow-[0_0_30px_rgba(37,99,235,.35)] transition hover:scale-105">
              Get Started
            </a>
          </div>
        ...
        {isMenuOpen && (
          <div className="mx-auto mt-3 w-full max-w-7xl rounded-2xl border border-white/10 bg-[#07101d]/95 p-4 shadow-2xl backdrop-blur-2xl md:hidden">
                ...
                item.href.startsWith("#") ? (
                  <button
                    ...
                    onClick={() => {
                      ...
                      trackCta(item.label.toLowerCase().replaceAll(" ", "_"), "mobile_menu", item.label === "Pricing" ? "pricing_click" : "cta_click");
                      setIsMenuOpen(false);
                      window.setTimeout(() => scrollToPageSection(sectionId), 20);
                    }}
                  >
                    {item.label}
                  </button>
                  ...
                  <a
                    ...
                    onClick={() => {
                      trackCta(item.label.toLowerCase().replaceAll(" ", "_"), "mobile_menu", item.label === "Register" ? "register_click" : "cta_click");
                      setIsMenuOpen(false);
                    }}
                  >
                    {item.label}
            ...
            <div className="mt-4 grid gap-3">
              <a href="/register" className="rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 px-4 py-3 text-center text-sm font-semibold" onClick={() => { trackCta("get_started", "mobile_menu", "register_click"); setIsMenuOpen(false); }}>Get Started</a>
            </div>
          </div>
        )}
      </header>

      <section id="home" className="relative flex min-h-[720px] scroll-mt-28 items-center overflow-hidden px-4 pb-8 pt-28 sm:min-h-screen sm:pt-52 lg:pt-40">
        ...
        <div className="relative z-10 mx-auto w-full max-w-7xl">
          <div className="grid w-full items-center gap-14 lg:grid-cols-2 lg:gap-20">
            <div className="mx-auto flex w-full max-w-2xl flex-col items-center text-center lg:mx-0 lg:items-start lg:text-left">
              ...
              <h1 className="mt-6 text-3xl font-black leading-[1.08] tracking-tight min-[390px]:text-4xl sm:mt-7 sm:text-5xl md:text-6xl lg:text-[68px]">
                Automate Your
              ...
              <p className="mt-5 max-w-xl text-sm leading-7 text-slate-400 sm:mt-7 sm:text-lg sm:leading-8">
              ...
              <div className="mt-7 flex w-full flex-col justify-center gap-3 sm:mt-9 sm:w-auto sm:flex-row sm:gap-4 lg:justify-start">
                <a href="#pricing" onClick={() => trackCta("start_free_trial", "hero_trial", "pricing_click")} className="rounded-2xl bg-gradient-to-r from-blue-600 to-indigo-600 px-6 py-3.5 text-center font-semibold shadow-[0_0_35px_rgba(37,99,235,.45)] transition hover:scale-105 sm:px-8 sm:py-4">Start Free Trial +</a>
                <a href={leadFormLink} onClick={() => trackCta("book_demo", "hero_demo", "demo_click")} className="rounded-2xl border border-slate-700 bg-white/5 px-6 py-3.5 text-center font-semibold backdrop-blur-xl transition hover:border-blue-500 hover:bg-white/10 sm:px-8 sm:py-4">Book Demo</a>
              </div>


              <div className="mt-6 w-full max-w-xl sm:mt-8">
                <div className="grid gap-2 text-xs font-semibold text-slate-300 sm:grid-cols-3 sm:gap-3">
  ...
  {socialIcons.map(({ Icon, className, orbitClass }) => (
      ...
      <section id="features" className="scroll-mt-28 px-4 pb-8 pt-8">
        ...
        <div className="mx-auto grid w-full max-w-7xl items-center gap-8 lg:grid-cols-[1.2fr_0.88fr_0.82fr] lg:gap-6">
          ...
          <div className="text-center lg:text-left">
            ...
            <a
              href="#ai-live-demo"
              onClick={() => {
                setAiDemoStarted(true);
                setVisibleAiMessages(1);
                trackCta("see_ai_in_action", "automation_section");
                window.setTimeout(() => {
                  aiChatRef.current?.scrollIntoView({ behavior: "smooth", block: "center" });
                }, 50);
              }}
              className="mt-8 inline-flex rounded-lg bg-gradient-to-r from-violet-600 to-blue-600 px-7 py-3 text-sm font-bold shadow-[0_0_32px_rgba(124,58,237,.32)] transition hover:-translate-y-1"
            >
              See AI in Action +
            </a>
          </div>
          ...
          <div
            id="ai-live-demo"
            ref={aiChatRef}
            className={`scroll-mt-32 rounded-3xl border bg-[#0B1220]/95 p-5 shadow-[0_0_50px_rgba(59,130,246,.25)] backdrop-blur-2xl transition duration-300 hover:-translate-y-2 ${
              aiDemoStarted ? "border-blue-400/50 ring-2 ring-blue-400/20" : "border-white/10 hover:border-blue-500/40"
            }`}
          >
            ...
            <div ref={aiScrollRef} className="mt-5 max-h-[285px] space-y-4 overflow-y-auto pr-2">
              {aiDemoMessages.slice(0, visibleAiMessages).map((message, index) => (
                <div
                  key={`${message.role}-${index}`}
                  className={
                    message.role === "user"
                      ? "ml-auto max-w-[82%] rounded-2xl rounded-br-md bg-gradient-to-r from-blue-600 to-indigo-600 p-4 text-sm leading-6 text-white"
                      : "max-w-[90%] rounded-2xl rounded-bl-md bg-slate-800 p-4 text-sm leading-6 text-slate-300"
                  }
                >
                  {message.text}
                </div>
            ...
            <div className="mt-5 flex items-center gap-2">
              <span className="text-xs text-slate-400">
                {aiDemoStarted && visibleAiMessages >= aiDemoMessages.length ? "Automation preview complete" : "AI is typing"}
              </span>
              {(!aiDemoStarted || visibleAiMessages < aiDemoMessages.length) && (
      ...
      <section id="about" className="scroll-mt-28 px-4 py-10">
      ...
      <section className="px-4 pb-8 pt-8">
        <div className="mx-auto w-full max-w-7xl">
          <div className="text-center">
            ...
            <h2 className="mt-3 text-3xl font-black sm:text-5xl">Get Started in 3 Simple Steps</h2>
          </div>
      ...
      <section id="pricing" className="scroll-mt-28 px-4 pb-16 pt-8">
        <div className="mx-auto w-full max-w-7xl">
          <div className="text-center">
          ...
          <div className="mt-10 grid gap-5 md:grid-cols-2 xl:grid-cols-4">
            {[
              ...
              <article key={plan.name} className="relative rounded-lg border border-white/10 bg-[#07101d]/85 p-6 shadow-[0_18px_55px_rgba(0,0,0,.22)] transition duration-300 hover:-translate-y-1 hover:border-blue-500/50 hover:bg-white/[0.05]">
                ...
                <a
                  href={leadFormLink}
                  onClick={() => trackCta(plan.name === "Enterprise" ? "talk_to_sales" : "pricing_plan_trial", `pricing_${plan.name.toLowerCase()}`, "demo_click")}
        ...
        <div className="mx-auto w-full max-w-7xl">
          <div className="mx-auto max-w-3xl text-center">
            ...
            <p className="mt-3 text-sm leading-7 text-slate-400">
              Clear answers about setup, real API approvals, AI replies, lead capture, and team control before you begin.
            </p>
          </div>

          <div className="mt-8 grid items-start gap-4 lg:grid-cols-2">
            {faqs.map((item) => (
              <article key={item.question} className="rounded-lg border border-white/10 bg-[#07101d]/85 p-5 shadow-[0_16px_45px_rgba(0,0,0,.18)] transition duration-300 hover:-translate-y-1 hover:border-blue-400/45 hover:bg-white/[0.05]">
          ...
          <div className="mt-5 rounded-lg border border-amber-300/15 bg-amber-300/10 p-4 text-sm leading-7 text-amber-50">
            Real Instagram, Facebook, Messenger, and WhatsApp automation depends on official provider API approval. Pasnex.ai can prepare the workspace first, then activate live messaging after approval and token setup.
          </div>
        </div>
      </section>

      <section id="blog" className="scroll-mt-28 px-4 pb-14 pt-2">
        <div className="mx-auto w-full max-w-7xl">
      ...
      <section id="contact" className="scroll-mt-28 px-4 pb-12 pt-4 sm:pb-20">
        <div className="relative mx-auto flex w-full max-w-7xl flex-col items-start justify-between gap-5 overflow-hidden rounded-lg border border-violet-500/20 bg-gradient-to-r from-violet-700/75 via-blue-700/45 to-[#07101d] px-5 py-6 shadow-[0_20px_70px_rgba(37,99,235,.2)] sm:px-8 md:flex-row md:items-center lg:pr-44">
          ...
          <div className="relative z-20 hidden items-center gap-3 md:flex lg:-translate-x-12">
            <a href={leadFormLink} onClick={() => trackCta("start_free_trial", "final_cta", "demo_click")} className="rounded-lg bg-gradient-to-r from-violet-600 to-blue-600 px-7 py-3 text-sm font-bold text-white shadow-[0_0_28px_rgba(37,99,235,.35)] transition hover:scale-[1.03]">Start 14-Day Free Trial +</a>
          ...
          <div className="relative z-20 grid w-full gap-3 md:hidden">
            <a href={leadFormLink} onClick={() => trackCta("start_free_trial", "final_cta_mobile", "demo_click")} className="rounded-lg bg-gradient-to-r from-violet-600 to-blue-600 px-5 py-3 text-center text-sm font-bold text-white shadow-[0_0_28px_rgba(37,99,235,.35)]">Start 14-Day Free Trial +</a>
            <a href={whatsappLink} onClick={() => trackCta("whatsapp", "final_cta_mobile")} target="_blank" rel="noreferrer" className="inline-flex items-center justify-center gap-2 rounded-lg border border-emerald-400/20 bg-emerald-400/10 px-5 py-3 text-sm font-bold text-emerald-200">
              <SiWhatsapp className="h-5 w-5" />
              <span>WhatsApp</span>
            </a>
          </div>
          <div className="relative z-20 grid w-full gap-3 sm:grid-cols-3 md:hidden">
            <a href={leadFormLink} onClick={() => trackCta("start_free_trial", "mobile_final_cta", "demo_click")} className="rounded-lg bg-gradient-to-r from-violet-600 to-blue-600 px-5 py-3 text-center text-sm font-bold text-white shadow-[0_0_28px_rgba(37,99,235,.35)]">Start 14-Day Free Trial +</a>
            <a href={leadFormLink} onClick={() => trackCta("book_demo", "mobile_final_cta", "demo_click")} className="inline-flex items-center justify-center rounded-lg border border-white/10 bg-white/[0.04] px-5 py-3 text-sm font-bold text-white">
              Book Demo
            </a>
      ...
      <section id="lead-form" className="px-4 pb-12 pt-2 sm:pb-16">
        <div className="mx-auto grid w-full max-w-7xl gap-6 rounded-lg border border-white/10 bg-[#07101d]/85 p-6 shadow-[0_20px_70px_rgba(0,0,0,.24)] lg:grid-cols-[0.85fr_1.15fr] lg:p-8">
          ...
          <form
            ...
            method="POST"
            onSubmit={() => {
              if (formRedirectRef.current) {
                formRedirectRef.current.value = `${window.location.origin}/thank-you`;
              }
              if (formReplyToRef.current && formEmailRef.current) {
                formReplyToRef.current.value = formEmailRef.current.value.trim();
              }
              if (formContactSummaryRef.current && formCountryCodeRef.current && formPhoneRef.current) {
                formContactSummaryRef.current.value = `${formCountryCodeRef.current.value} ${formPhoneRef.current.value}`;
              }
              trackCta("demo_form_submit", "lead_form", "demo_click");
            }}
            className="grid gap-4 rounded-lg border border-white/10 bg-white/[0.035] p-5"
          >
            <input type="hidden" name="_subject" value="New Pasnex.ai Demo Request" />
            <input type="hidden" name="_captcha" value="false" />
            <input type="hidden" name="_template" value="table" />
            <input type="text" name="_honey" className="hidden" tabIndex={-1} autoComplete="off" />
            <input ref={formReplyToRef} type="hidden" name="_replyto" />
            <input type="hidden" name="Request Type" value="Landing Page Demo Request" />
            <input ref={formContactSummaryRef} type="hidden" name="WhatsApp Contact" />
            <input type="hidden" name="Preferred Follow-up" value="Reply by email or WhatsApp within 24 hours" />
            <input ref={formRedirectRef} type="hidden" name="_next" defaultValue="https://pasnex.com/thank-you" />
            <div className="grid gap-4 sm:grid-cols-2">
              <input name="Full Name" required className="rounded-lg border border-white/10 bg-[#030712] px-4 py-3 text-sm text-white outline-none transition placeholder:text-slate-600 focus:border-blue-400" placeholder="Your name" />
              <input name="Business Name" required className="rounded-lg border border-white/10 bg-[#030712] px-4 py-3 text-sm text-white outline-none transition placeholder:text-slate-600 focus:border-blue-400" placeholder="Business name" />
            </div>
            <div className="grid gap-4 sm:grid-cols-2">
              <input ref={formEmailRef} name="Work Email" type="email" required className="rounded-lg border border-white/10 bg-[#030712] px-4 py-3 text-sm text-white outline-none transition placeholder:text-slate-600 focus:border-blue-400" placeholder="Work email" />
              <select name="Primary Channel" required className="rounded-lg border border-white/10 bg-[#030712] px-4 py-3 text-sm text-white outline-none transition focus:border-blue-400" defaultValue="">
            ...
            <div className="grid gap-4 sm:grid-cols-[220px_1fr]">
              <select ref={formCountryCodeRef} name="Country Code" required className="rounded-lg border border-white/10 bg-[#030712] px-4 py-3 text-sm text-white outline-none transition focus:border-blue-400" defaultValue="+91">
              ...
              <input
                ...
                className="rounded-lg border border-white/10 bg-[#030712] px-4 py-3 text-sm text-white outline-none transition placeholder:text-slate-600 focus:border-blue-400"
                placeholder="Phone / WhatsApp number"
                title="Enter only numbers, 7 to 15 digits"
              />
            </div>
            <textarea name="Automation Requirement" required rows={4} className="resize-none rounded-lg border border-white/10 bg-[#030712] px-4 py-3 text-sm text-white outline-none transition placeholder:text-slate-600 focus:border-blue-400" placeholder="What do you want to automate?" />
