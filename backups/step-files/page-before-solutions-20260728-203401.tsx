...
const useCases = [
  {
    ...
    title: "E-commerce",
    text: "Recover abandoned carts, answer product questions, and turn comments into qualified sales conversations.",
  },
  {
    Icon: HiOutlineBuildingOffice2,
    title: "Real Estate",
    text: "Capture property enquiries, qualify buyers, schedule site visits, and follow up automatically.",
  },
  {
    Icon: HiOutlineAcademicCap,
    title: "Education",
    text: "Respond to course enquiries, collect student details, and move prospects into counselling flows.",
  },
  {
    Icon: HiOutlineChatBubbleLeftRight,
    title: "Local Services",
    text: "Handle bookings, FAQs, estimates, and support messages across social channels without missing leads.",
  },
];
        ...
        <div className="mx-auto w-full max-w-7xl">
          ...
          <div className="mt-8 grid items-start gap-4 md:grid-cols-2 xl:grid-cols-6">
            {[
              ...
              <article key={title} data-feature-card className="group flex h-[320px] cursor-pointer flex-col rounded-lg border border-white/10 bg-[#07101d]/90 p-5 text-left shadow-[0_14px_45px_rgba(0,0,0,.22)] transition duration-300 hover:-translate-y-1 hover:border-violet-400/50 hover:bg-white/[0.055] hover:shadow-[0_18px_55px_rgba(124,58,237,.18)] xl:h-[350px]">
      ...
      <section className="px-4 pb-10 pt-4">
        <div className="mx-auto w-full max-w-7xl">
          <div className="grid gap-6 lg:grid-cols-[0.9fr_1.5fr] lg:items-end">
            <div>
              <p className="text-sm font-bold uppercase tracking-[0.3em] text-blue-400 sm:text-base">Built for growth teams</p>
              <h2 className="mt-3 text-3xl font-black leading-tight sm:text-4xl">Automation workflows for every lead-driven business</h2>
            </div>
            <p className="text-sm leading-7 text-slate-400">
              Turn social conversations into structured workflows for sales, support, bookings, and follow-ups across the channels your customers already use.
            </p>
          </div>

          <div className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            {useCases.map(({ Icon, title, text }) => (
              <article key={title} className="group rounded-lg border border-white/10 bg-[#07101d]/85 p-6 shadow-[0_16px_45px_rgba(0,0,0,.2)] transition duration-300 hover:-translate-y-1 hover:border-blue-400/50 hover:bg-white/[0.055]">
              ...
              <div key={group.title}>
                ...
                <ul className="mt-3 space-y-2 text-sm text-slate-500">
                  {group.links.map((link) => (
                    <li key={link}><a href={link === "Privacy Policy" ? "/privacy" : link === "Pricing" ? "#pricing" : link === "Features" ? "#features" : link === "Integrations" ? "#integrations" : link === "Contact Us" ? "#contact" : link === "About Us" ? "#about" : link === "Blog" ? "#blog" : "#"} className="transition hover:text-white">{link}</a></li>
                  ))}
                </ul>
              </div>
            ))}
